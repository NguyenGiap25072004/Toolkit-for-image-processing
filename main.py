...
from processing import enhancement, restoration, morphology, segmentation, recognition, compression, color_processing

class MainWindow(ThemedTk):
    def __init__(self):
        super().__init__(theme="arc")  # Chọn theme từ ttkthemes
        ...
        # Bây giờ mới tạo ProcessingPanel
    ...
    def apply_compression(self, func_name):
        self.apply_processing(self.compression_functions, func_name)

    def apply_color_processing(self, func_name):
        self.apply_processing(self.color_processing_functions, func_name)
        ...
        if self.image_viewer.original_image:
            try:
                if func_name in ["Adjust Brightness", "Adjust Contrast", "Adjust Saturation"]:
                  ...
                  processed_image = functions[func_name](self.image_viewer.original_image, factor)
                else:
